...
var STATE_FIRST = 0, STATE_PREAUTH = 1, STATE_AUTH = 2, STATE_AUTHED = 3, STATE_BIND = 4, STATE_SESSION = 5, STATE_ONLINE = 6, STATE_TERM = 7, STATE_OVER = 8;

/*
 * jid : [String] jabber id of user (e.g. 'user@example.com/office') password :
 * [String] password bosh : [String] url of the bosh-server (e.g.
 * 'http://localhost:5280/http-bind/') route : [String] (optional) route
 * attribute [if used] for connecting to xmpp server
 */
function nxbClient(jid, password, bosh, wait, hold, callback) {
	...
	// bool to check whether sendPending is scheduled on nextTick to send
	// pending stanzas
	...
	this.sendHttp = function(body) {
		var that = this;
		this.chold++;
		autil.xmlHttpRequest(this.options, function(err, response) {
	...
	this.pError = function(ss) {
		...
		callback(self.sess_attr.jid,  false);
		// emit "error" event
		//this.emit("error", ss);
	...
	this.handle = function(err, response) {
		...
		if (err) {
			autil.logIt("ERROR", this.sess_attr.jid + " no response "
					+ response);
		...
		if (serror = body.getChild("error", NS_STREAM)) {
			...
			/*
			 * No need to terminate as stream already closed by xmppserver and
			 * hence bosh-server but to inform other asynch methods not to send
			 * messages any more change the state
			 */
			state = STATE_TERM;
...
//				this.emit("offline", "Session termination by server "
//						+ body.toString());
		...
		if (this.state == STATE_AUTH) {
			autil.logIt("DEBUG", "STATE_AUTH with body: "
					+ body.getChild("success",
							"urn:ietf:params:xml:ns:xmpp-sasl")
					+ " and NS_CLIENT: " + NS_CLIENT);

			if (success = body.getChild("success",
					"urn:ietf:params:xml:ns:xmpp-sasl")) {
				autil.logIt("DEBUG", "Authentication Success:  "
						+ this.sess_attr.jid);
				this.state = STATE_AUTHED;
				this.restartStream(); // restart stream
			} else if (failure = body.getChild("failure", NS_CLIENT)) {
				this.pError("Authentication Failure: " + this.sess_attr.jid
						+ body);
			} else {
		...
		if (this.state == STATE_AUTHED) {
			...
			if (features = body.getChild("features", NS_STREAM)) {
				...
				if (features.getChild("bind", NS_XMPP_BIND)) {
					...
					this.bindResource(this.sess_attr.jid.resource); // bind
					// resource
				} else {
		...
		if (this.state == STATE_BIND) {
			...
			if (iq = body.getChild("iq", NS_CLIENT)) {
				if (iq.attrs.id == "bind_1" && iq.attrs.type == "result") {
					var cjid = iq.getChild("bind", NS_XMPP_BIND).getChild(
							"jid", NS_XMPP_BIND).getText();
					this.sess_attr.jid.resource = cjid
							.substr(cjid.indexOf("/") + 1);

					if (this.sessionSupport) {
						var iq = new ltx.Element("iq", {
							to : this.sess_attr["jid"].domain,
							type : "set",
							id : "sess_1"
						});
...
//					this.processCallback();
					callback(self.sess_attr.jid,  self.sess_attr.rid - 1, self.sess_attr.sid);
					this.state = STATE_OVER;
				} else {
					this
							.pError("iq stanza error session establishment : "
									+ iq);
				}
			} else {
				this.sendXml();
...
//		var jid = self.sess_attr.jid;
//		var rid = self.sess_attr.rid - 1;
//		var sid = self.sess_attr.sid;
		callback(self.sess_attr.jid,  self.sess_attr.rid - 1, self.sess_attr.sid);
	};
...
//	};
	...
	this.getPlain = function() {
		...
		return autil.encode64(authzid + "\u0000" + authcid + "\u0000"
				+ password);
	};
	...
	// sends an ltx-xml element by wrapping it into body element[change it to
	// array thing]
	...
	this.sendMessage = function(to, mbody, type) {
		var message = new ltx.Element("message", {
			to : to,
			from : this.sess_attr.jid.toString(),
			type : type || "chat",
			"xml:lang" : "en"
		});
	...
	this.sendPending = function() {
		// send only if u have something to send or u need to poll the
		// bosh-server
